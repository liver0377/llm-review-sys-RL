...
echo "External Deployment (vLLM Serve)"
...
swift rlhf \
    ...
    --output_dir models/grpo_qwen3_9b_grm_external \
    --tuner_type full \
    ...
    --sleep_level 1 \
    --report_to wandb \
    --run_name grpo_qwen3_9b_grm_external_v1 \
    2>&1 | tee "$LOG_FILE"
...
echo ""
echo "Models:"
echo "  ✓ Policy Model:      models/grpo_qwen3_9b_grm_external"
...
echo "  - Expected speedup: ~12% faster than internal deployment"
echo "  - RM evaluation time: ~2s (vs ~7.6s in internal)"
echo ""
echo "To evaluate the model:"
echo "  python eval/eval.py --model_name grpo_qwen3_9b_grm_external"
echo ""
