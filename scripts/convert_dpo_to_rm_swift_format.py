...
Format: Swift RM expected format (messages + rejected_messages)
"""
...
def convert_dpo_to_rm_swift_format(
    dpo_train_path,
    dpo_val_path,
    output_dir,
    ...
    """Convert DPO data to Swift RM format with FULL paper content"""
    
    print("=" * 70)
    print("Converting DPO to Swift RM format with FULL paper content")
    print("=" * 70)
    ...
    for item in tqdm(train_data, desc="Converting train"):
        ...
        # Swift RM format: messages + rejected_messages
        rm_item = {
            "messages": [
    ...
    print(f"  - {val_output}")
    print("\nData format:")
    print("  - messages: [{role: 'user', content: query}, {role: 'assistant', content: chosen}]")
    print("  - rejected_messages: [{role: 'user', content: query}, {role: 'assistant', content: rejected}]")
    print("\nRecommended training config:")
    print("  max_length=16384  (covers 99.4% samples, P99=15168)")
    print("=" * 70)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Convert DPO to Swift RM format with full paper content")
    ...
    convert_dpo_to_rm_swift_format(
